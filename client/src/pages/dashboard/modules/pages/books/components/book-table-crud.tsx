...
export function BookTableCrud({ books }: BookTableProps) {
  ...
  const [, setSelectedBook] = useState<BookI | null>(null);
...
*/
  ...
  const renderBookCard = (book: BookI) => (
    <Card
      key={book.id}
      className="mb-4 border-[#3e3e40] bg-[#0e0e0e] text-[#C7C7CC]"
    >
      <CardHeader>
      ...
      <CardContent>
        ...
        <p>
          <strong>Subcategoría:</strong> {book.subcategory}
        </p>
        <Button
          onClick={() => handleEdit(book)}
          className="mt-2 bg-[#FFBC24] text-[#010101] hover:bg-[#FFBC24]/90"
        >
          Editar
        </Button>
        <Button
  ...
  return (
    <>
      ...
      <div className="md:hidden">{currentBooks.map(renderBookCard)}</div>
