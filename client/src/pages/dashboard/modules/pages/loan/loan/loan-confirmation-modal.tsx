...
export function LoanConfirmationModal({
  ...
  const generateTimeOptions = () => {
    ...
    currentTime = setMinutes(
      currentTime,
      Math.ceil(currentTime.getMinutes() / 30) * 30
    );
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await loanApi.updateReservation(
        reservation.id,
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Hubo un problema al crear el préstamo. Por favor, inténtalo de nuevo.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="loan-modal__content">
        <DialogHeader>
          <DialogTitle className="loan-modal__title">
            Confirmar conversión a préstamo
          </DialogTitle>
        ...
        <div className="loan-modal__grid">
          ...
          <div className="flex items-center gap-2 mb-2">
            <Label htmlFor="loanTime" className="loan-modal__label">
              Hora de finalización del préstamo:
            </Label>
