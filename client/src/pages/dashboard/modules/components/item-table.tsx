...
export function ItemTable({
  ...
  const [isLoanConfirmationModalOpen, setIsLoanConfirmationModalOpen] =
    useState(false);
  const [isReservationStatusModalOpen, setIsReservationStatusModalOpen] =
    useState(false);
  ...
  const [isDeleteModalOpenDonations, setIsDeleteModalOpenDonations] =
    useState(false);
  ...
  const handleReserve = async (item: BookI) => {
    try {
      ...
      toast({
        title: "Error",
        description:
          "No se pudieron obtener las copias del libro. Por favor, inténtalo de nuevo.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description:
          error instanceof Error ? error.message : "Error al eliminar el libro",
        variant: "destructive",
      });
    }
  };
  ...
  const handleConfirmReservation = async (
    ...
    try {
      ...
      toast({
        ...
        description: `Has reservado "${selectedItem.title}" (Copia: ${
          selectedCopy.code || "N/A"
        }) para el ${format(reservationDate, "dd/MM/yyyy HH:mm", {
          locale: es,
        })}.`,
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Hubo un problema al crear la reserva. Por favor, inténtalo de nuevo.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleConvertToLoan = (item: Item) => {
    if ("copies" in item && item.status === ReservationStatus.PENDING) {
      ...
      console.error(
        "Attempted to convert a non-pending reservation or non-reservation item to loan"
      );
      toast({
        title: "Error",
        description:
          "Solo se pueden convertir a préstamo las reservas pendientes.",
        variant: "destructive",
      });
    }
  };
    ...
    try {
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Error al actualizar el Donador",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="item-table">
      ...
      {viewMode === "loan" && selectedItem && "copies" in selectedItem && (
        <>
          <LoanConfirmationModal
            ...
            bookTitle={
              selectedItem.copies[0]?.book.title || "Libro desconocido"
            }
          />
      ...
      {(viewMode === "loan" || viewMode === "loan-history") &&
        selectedItem &&
        "loanDate" in selectedItem && (
          <LoanStatusModal
